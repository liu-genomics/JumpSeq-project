...
rownames(ave)=c("Nano_seal", "Strong Tab-seq")
...
```{r, echo=F, fig.cap="The  reads distribution around the center site C/G with 10 bp downstream and upstream. The sampls is the combined eleven 48ng replicates. The central three nucleotides are tCt (plus strand) or tGt (minus strand) "}
...
#pdf("D:\\Dropbox\\JumpSeqProject\\Figure\\Jump_distribution_centerTCGT_48ng_11sampleCombined.pdf")
par(mfrow=c(1,2), oma = c(0, 0, 2, 0))

plot(seq(-10, 10, by=1)+1,plus.48ng[91:111,1], type="l", ylab="Coverage", xlab="Distance", main="Plus Strand")
abline(v=0, col=2)
plot(seq(-10, 10, by=1)-1,minus.48ng[91:111,1], type="l", ylab="Coverage", xlab="Distance", main="Minus Strand")
abline(v=0, col=2)
...
pdf("D:\\Dropbox\\JumpSeqProject\\Figure\\Jump_distribution_allbackground_48ng_11sampleCombined.pdf")
par(mfrow=c(1,2))
...
### Overlap with 48ng samples 


```{r, echo=F}
overlap=read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5hmC_0.6ng\\overlap", header=F)
reads.count=overlap[c(1,3,4,6)]
par(mfrow=c(1,2))
rep1=(reads.count[1:18,2]/reads.count[1:18,1]+reads.count[19:36,2]/reads.count[19:36,1])/2
rep1.ave=numeric()
rep2=(reads.count[37:54,2]/reads.count[37:54,1]+reads.count[55:72,2]/reads.count[55:72,1])/2
rep2.ave=numeric()
for (i in 1:9)
{
  rep1.ave[i]=mean(rep1[(i-1)*2+1], rep1[i*2])
  rep2.ave[i]=mean(rep2[(i-1)*2+1], rep2[i*2])
}
barplot(rep1.ave, ylim=c(0,1), ylab="Proportation of 0.6ng 20 bp windows", main=paste("mean of", round(mean(rep1.ave), digits=4)))
barplot(rep2.ave, ylim=c(0,1), ylab="Proportation of 0.6ng 20 bp windows", main=paste("mean of", round(mean(rep2.ave), digits=4)))
```

* overlap between 0.6ng single sample 20 bp windows with reads and 48ng single sample 20 bp windows with reads. 
...
plot(hmc.ratio, num.reads.plus1/num.reads.plus1[1], xlim=c(0,1), ylim=c(0, 110), type="p", ylab="Fold change of reads", xlab="Ratio of hmC", xaxt="n", main="")
axis(1, at =hmc.ratio, las=0, cex.axis=0.5)
...
### top 2 samples with most reads 

```{r, echo=T, eval=F}
CHe-LHu-10S-PE75-lu-4_S4_R1_001_jump_TGACTCG.umi_encoded_adaptor_removed_no_mismatch_sorted_dedup_TP01.bam 354531   CHe-LHu-10S-PE75-lu-6_S6_R1_001_jump_TGACTCG.umi_encoded_adaptor_removed_no_mismatch_sorted_dedup_TP15.bam 92623
```



```{r, echo=F}
percent=matrix(nrow=2, ncol=4)
percent[,1]=c(0.0854,0.0194)
percent[,2]=c(0.1382,0.0257)

percent[,3]=c(0.0771, 0.0194)
percent[,4]=c(0.1151,0.0257)


colnames(percent)=rep(c("Sample 1", "Sample 2"), each=2)

#par(mfrow=c(1,2))
par(mfrow=c(1,2),oma = c(0, 0, 2, 0))
barplot(percent[,c(1,3)], beside=T, col=c("red", "darkblue"), legend=c("Jump-seq peaks", "Random windows"), ylim=c(0,0.3),
xlab="Figure 1A", main="", ylab="Proportion of Nano-seal peaks", cex.names=0.5)

barplot(percent[,c(2,4)], beside=T, col=c("red", "darkblue"), legend=c("Jump-seq peaks", "Random windows"), ylim=c(0,0.4),
xlab="Figure 1B", main="", ylab="Proportion of Tab-seq peaks", cex.names=0.5)
mtext("Enrichemnt of two 0.5ng mES samples", outer = TRUE, cex = 1.5)
```

* Windows with reads are chosen and extended to 20 bp 

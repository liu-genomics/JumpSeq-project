...
```{r, echo=F, fig.cap=" Reads correlations in 1kb windows between two 5hmC replicates. log2 transformation in calculation of correlations  of number of reads", warning=F}
reads=read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5hmC\\mm9.genome.1kb.window.bed.CHe-Lu-1_S12_L005_R1_001.plus.21.bp.bed.He-Lu-lu-1-48ng_S1_L006_R1_001.plus.21.bp.win.bed", header=F)
...
reads1=as.numeric(as.character(read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5hmC\\first.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.second.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.bed.reads.1", header=F)[,1]))
reads1[is.na(reads1)]=0
reads2=as.numeric(as.character(read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5hmC\\first.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.second.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.bed.reads.2", header=F)[,1]))
reads2[is.na(reads2)]=0
...
barplot(overlap[,6], beside=T,  ylim=c(0, max(overlap)), col=c("red", "darkblue"), 
ylab="# Tab-seq peaks overlaping with other peaks", main="48ng Jump-seq vs 50ng nano-seal", xlab="", cex.names=0.7, legend=c("Jump-seq peaks", "nano-seal peaks"))
...
reads1=as.numeric(as.character(read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5mC\\20160811_5mC_Jump_Seq_2.4ng.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.He-Lu-10_5mc-jump-2.4ng-S10_L004_R1_001.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.bed.reads.1", header=F)[,1]))
reads1[is.na(reads1)]=0
reads2=as.numeric(as.character(read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\5mC\\20160811_5mC_Jump_Seq_2.4ng.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.He-Lu-10_5mc-jump-2.4ng-S10_L004_R1_001.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.bed.reads.2", header=F)[,1]))
reads2[is.na(reads2)]=0
...
scale_hmc_combine=scale(as.matrix(first_hmc_combine)) 
...
######### plot 1*3 figure 
par(mfrow=c(1,3))
plot(scale_hmc_combine[1:5,1], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="normalized reads count", col=1, main=" 5hmC-Maternal", cex.main=0.8, cex.lab=1, lwd=2)
...
plot(scale_hmc_combine[6:10,1], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="", col=1, main=" 5hmC-Paternal", cex.main=0.8, cex.lab=0.6, lwd=2)
for (i in 2:num.regions)
...
par(mfrow=c(1,1))
hmc_combine_region=rowSums(first_hmc_combine)
scale_hmc_combine_region=scale(hmc_combine_region)
min.limit=min(scale_hmc_combine_region); max.limit=max(scale_hmc_combine_region)
plot(scale_hmc_combine_region[male.index], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="normalized reads count", col=1, main="5hmC at different stages", cex.main=0.8, cex.lab=1, lty=1, pch=16, lwd=2)
lines(scale_hmc_combine_region[female.index], col=2, type="o", lty=1, pch=16, lwd=2)
legend(1, max.limit, c("Paternal", "Maternal"), col=seq(1,2), lty=rep(1,2), ncol=2, cex=0.65)
...
scale_hmc_combine=scale(as.matrix(second_hmc_combine))
...
plot(scale_hmc_combine[6:10,1], ylim=c(-2, 2), type="o", xaxt='n', xlab="", ylab="", col=1, main=" 5hmC-Paternal", cex.main=0.8, cex.lab=0.6, lwd=2)
for (i in 2:num.regions)
  lines(scale_hmc_combine[6:10,i], col=i, type="o", lwd=2)
legend(1, 3, regions, col=seq(1,num.regions), lty=rep(1,num.regions), ncol=3, cex=0.45)
axis.labels <-c("MII", "PN1","PN2","PN3","PN4")
...
plot(scale_hmc_combine[11:15,1], ylim=c(-2, 2),  type="o", xaxt='n', xlab="", ylab="", col=1, main="", cex.main=0.8, cex.lab=0.6)
for (i in 2:num.regions)
  lines(scale_hmc_combine[11:15,i], col=i, type="o")
legend(1, 3, regions, col=seq(1,num.regions), lty=rep(1,num.regions), ncol=3, cex=0.45)
axis.labels <-c("PN5","1cell","2cellearly", "2celllate", "ES")
...
################ plot with regions combined 
par(mfrow=c(1,1))
hmc_combine_region=rowSums(second_hmc_combine)
scale_hmc_combine_region=scale(hmc_combine_region)
min.limit=min(scale_hmc_combine_region); max.limit=max(scale_hmc_combine_region)
plot(scale_hmc_combine_region[male.index], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="normalized reads count", col=1, main="5hmC at different stages", cex.main=0.8, cex.lab=1, lty=1, pch=16, lwd=2)
lines(scale_hmc_combine_region[female.index], col=2, type="o", lty=1, pch=16, lwd=2)
...
scale_mc_combine=scale(as.matrix(first_mc_combine)) 
min.limit=min(scale_mc_combine); max.limit=max(scale_mc_combine)

######### plot 1*3 figure 
par(mfrow=c(1,3))
plot(scale_mc_combine[1:5,1], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="normalized reads count", col=1, main=" 5mC-Maternal", cex.main=0.8, cex.lab=1, lwd=2)
for (i in 2:num.regions)
  lines(scale_mc_combine[1:5,i], col=i, type="o", lwd=2)
legend(1, max.limit, regions, col=seq(1,num.regions), lty=rep(1,num.regions), ncol=2, cex=0.65)
axis.labels <-c("MII", "PN1","PN2","PN3","PN4")
axis(1, at=seq(1,5), labels=axis.labels, cex.axis=0.8, las=2)

plot(scale_mc_combine[6:10,1], ylim=c(-2, 3), type="o", xaxt='n', xlab="", ylab="", col=1, main=" 5mC-Paternal", cex.main=0.8, cex.lab=0.6, lwd=2)
for (i in 2:num.regions)
  lines(scale_mc_combine[6:10,i], col=i, type="o", lwd=2)
axis.labels <-c("MII", "PN1","PN2","PN3","PN4")
axis(1, at=seq(1,5), labels=axis.labels, cex.axis=0.8, las=2)

plot(scale_mc_combine[11:15,1], ylim=c(-2, 3),  type="o", xaxt='n', xlab="", ylab="", col=1, main="", cex.main=0.8, cex.lab=0.6)
for (i in 2:num.regions)
  lines(scale_mc_combine[11:15,i], col=i, type="o")
axis.labels <-c("PN5","1cell","2cellearly", "2celllate", "ES")
axis(1, at=seq(1,5), labels=axis.labels, cex.axis=0.8, las=2)

################# plot with regions combined 
par(mfrow=c(1,1))
mc_combine_region=rowSums(first_mc_combine)
scale_mc_combine_region=scale(mc_combine_region)
min.limit=min(scale_mc_combine_region); max.limit=max(scale_mc_combine_region)
plot(scale_mc_combine_region[male.index], ylim=c(min.limit,max.limit), type="o", xaxt='n', xlab="", ylab="normalized reads count", col=1, main="5mC at different stages", cex.main=0.8, cex.lab=1, lty=1, pch=16, lwd=2)
lines(scale_mc_combine_region[female.index], col=2, type="o", lty=1, pch=16, lwd=2)

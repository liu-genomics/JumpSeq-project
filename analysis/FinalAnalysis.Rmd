...
```{r, echo=F}
path="/Users/han24/Documents/Research/Jump-Seq/"
```
...
```{r, echo=F}
plus.48ng=read.table(paste(path,"JumpSeq-project/data/20161115_5hmC_Jump_Seq_2hmC_spike_in.mm9.umi_encoded_adaptor_removed_no_mismatch.sorted.dedup_GSM882244_FDR_0.0484_plus_with_1_coverage.ave.perbase", sep="/"), header=F)
...
reads=read.table(paste(path, "5hmC/mm9.genome.1kb.window.bed.CHe-Lu-1_S12_L005_R1_001.plus.21.bp.bed.He-Lu-lu-1-48ng_S1_L006_R1_001.plus.21.bp.win.bed", sep="/"), header=F)
...
reads1=as.numeric(as.character(read.table(paste(path,"5hmC/first.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.second.six.sample.sorted.bam.cove.base.level.extend.bed.with.reads.bed.reads.1", sep="/"), header=F)[,1]))
reads1[is.na(reads1)]=0
...
reads1=as.numeric(as.character(read.table(paste(path,"5mC/20160811_5mC_Jump_Seq_2.4ng.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.He-Lu-10_5mc-jump-2.4ng-S10_L004_R1_001.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.bed.reads.1", sep="/"), header=F)[,1]))
reads1[is.na(reads1)]=0
reads2=as.numeric(as.character(read.table(paste(path,"5mC/20160811_5mC_Jump_Seq_2.4ng.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.He-Lu-10_5mc-jump-2.4ng-S10_L004_R1_001.minus.21.bp.win.5prime.pstv.cove.bed.numberC.combine.pvalue.bed.reads.2", sep="/"), header=F)[,1]))
reads2[is.na(reads2)]=0
...
scale.reads=read.csv(paste(path, "20180113/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
new.scale.reads=read.csv(paste(path,"20180420/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
...
```{r, echo=F}
############## replicate 2 ####################
second.rep.scale.reads=read.csv(paste(path,"20180228/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
new.scale.reads=read.csv(paste(path, "20180420/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
...
```{r, echo=F}
############# maternal 
scale.reads=read.csv(paste(path, "20180420/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
...
```{r, echo=F}
############ paternal 
scale.reads=read.csv(paste(path, "20180227/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
new.scale.reads=read.csv(paste(path,"20180420/scale.reads.xiaolong.annotation.use.large.as.baseline.csv", sep="/"))
...
############### Maternal 
scale.reads=read.csv(paste(path, "20180227/scale.reads.xiaolong.annotation.csv", sep="/"))

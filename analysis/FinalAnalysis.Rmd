...
plot(log2(reads.effect[,1]), log2(reads.effect[,2]), pch=16, xlab="Rep 1", ylab="Rep 2", main="48ng 5hmC")
```
...
reads2[is.na(reads2)]=0
plot(log2(reads1), log2(reads2), pch=16, xlab="Six samples combined", ylab="The rest six samples", main="48ng combine 5hmC")
```
...
rownames(overlap)=c("Jump_seq", "Nano_seal")
kable(overlap, caption="Number of Tab-seq peaks recovered by Jump_seq and nano-seal")

barplot(overlap, beside=T,  ylim=c(0, max(overlap)), col=c("red", "darkblue"), xlim=c(1, 25), 
ylab="# Tab-seq peaks overlaping with other peaks", main="", xlab="", cex.names=0.7, legend=c("Jump-seq peaks", "nano-seal peaks"))
...
```{r, echo=F, fig.cap="Normalized reads count of 5hmC in different regions at different stages. Solid lineas are paternal  and and dotted liner are maternal and they both merge together after PN 5"}
...
scale_first_hmc_combine=scale(as.matrix(first_hmc_combine)) 
...
```{r, echo=F, fig.cap="Normalized reads count of 5mC in different regions at different stages. Solid lineas are paternal  and and dotted liner are maternal and they both merge together after PN 5"}
############# maternal 

...
```{r, echo=F, warning=F}
second.rep.scale.reads=read.csv("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180228\\scale.reads.csv")
...
```{r, echo=F, eval=F}
region.reads=read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180227\\important.region.reads", header=F)[,-1]
spikein.reads=read.table("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180227\\spike.in.reads", header=F)
reads.summary=as.matrix(cbind(region.reads, as.numeric(spikein.reads[,2])))
rownames(reads.summary)=c("mc-1cell", "mc-2cellearly", "mc-MII", "mc-sperm", "mc-sperm-repeat", "mc-ES",  "mc-PN1female", "mc-PN1male", "mc-PN2female", "mc-PN2male", "mc-PN3female", "mc-PN3male", "mc-PN4female", "mc-PN4male")
colnames(reads.summary)=c("exon", "intron", "3UTR", "5UTR", "CpGIsland", "transposable", "promoter", "enhancer", "spikein")
scale.factor=reads.summary[,9]/min(reads.summary[,9])
scale.reads=matrix(nrow=nrow(region.reads), ncol=8)
for (i in 1:nrow(region.reads))
...
rownames(scale.reads)=rownames(reads.summary)
colnames(scale.reads)=colnames(reads.summary)[1:8]
#write.csv(scale.reads, file="D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180227\\scale.reads.csv")
```

### Paternal 

```{r, echo=F, warning=F}
scale.reads=read.csv("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180227\\scale.reads.csv")
mc_male=rbind(scale.reads[4,], scale.reads[8,], scale.reads[10,], scale.reads[12,], scale.reads[14,], scale.reads[1:2,], scale.reads[6,] )
...
rownames(scale.male.mc)=c("Sperm", "PN1","PN2","PN3","PN4","1cell","2cellearly",  "ES")
...
legend(1, 4, c("exon", "intron", "3UTR", "5UTR", "CpGIsland", "trans", "promoter", "enhancer"), col=seq(1,8), lty=rep(1,8), ncol=3, cex=0.45)
axis.labels <-c("Sperm", "PN1","PN2","PN3","PN4","1cell","2cellearly",  "ES")
...
```{r, echo=F, warning=F}
scale.reads=read.csv("D:\\ResearchWork\\StatisticalGenetics\\JumpSeq\\20180227\\scale.reads.csv")
mc_female=rbind(scale.reads[3,], scale.reads[7,], scale.reads[9,], scale.reads[11,], scale.reads[13,], scale.reads[1:2,], scale.reads[6,] )
...
rownames(scale.female.mc)=c( "MII", "PN1","PN2","PN3","PN4","1cell","2cellearly",  "ES")
...
legend(1, 4, c("exon", "intron", "3UTR", "5UTR", "CpGIsland", "trans", "promoter", "enhancer"), col=seq(1,8), lty=rep(1,8), ncol=3, cex=0.45)
axis.labels <-c("MII", "PN1","PN2","PN3","PN4","1cell","2cellearly",  "ES")
axis(1, at=seq(1,8), labels=axis.labels, cex.axis=0.8, las=2)
